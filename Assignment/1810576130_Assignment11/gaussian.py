...
def processing():
    ...
    x = np.array([[1,2,1],
                ...
                [1,2,1]])/16
    print(x)
    mask[crow:crow+3,ccol:ccol+3] = 1
    
    
    img_mask = np.fft.fft2(mask)
    mask_sift = np.fft.fftshift(img_mask)
    mask_mag = 20*np.log(np.abs(mask_sift))
    
    #appy mask and inverse FFT
    ifft_shift = np.fft.ifftshift(fshift*mask_mag)
